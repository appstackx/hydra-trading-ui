...
| **Risk controls**           | A desk kill switch — thrown from the UI by entitled users (including the read-only risk user), persisted across refresh with no back door out (not even `?fresh=1`), audited both ways, and re-enforced in the service layer. A latched daily loss limit with hysteresis, enforced in the service layer too, both transitions audited. A basis-point fat-finger check on limit prices. |
| **Audit trail**             | Who did what, when — and for each trade, the exact quote on screen at the click. On-screen viewer with filters, RFC 4180 CSV export, persisted across refresh.                                                                                                                                                                                                                         |
| **Session persistence**     | Blotter, working orders and the audit trail survive a refresh; id sequences continue rather than reissue; resting orders resume matching. Orders carry an owner and cancel rights follow ownership. `?fresh=1` resets.                                                                                                                                                                 |
...
| Suite                                           | Count | Notes                                                      |
| ----------------------------------------------- | ----- | ---------------------------------------------------------- |
| Unit / component (Vitest + Testing Library)     | 586   | 98% statements, enforced by threshold                      |
| End-to-end (Playwright)                         | 180   | Desktop Chromium, desktop Firefox, mobile Chrome           |
